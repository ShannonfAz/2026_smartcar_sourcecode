...
// 备注信息     在指定坐标绘制一个像素点
//-------------------------------------------------------------------------------------------------------------------
void zf_device_ips200::draw_point(int16 x, int16 y, const uint16 color)
{
    x = std::min((int16)(width),std::max(x,(int16)0));
    y = std::min((int16)(height),std::max(y,(int16)0));
    if(nullptr != screen_base)
    {
        screen_base[y * width + x] = color;
    }
}
unsigned short* zf_device_ips200::returnscreenbase(){
    return screen_base;
}
...
// 备注信息     在指定两点之间绘制一条直线
//-------------------------------------------------------------------------------------------------------------------
void zf_device_ips200::draw_line (int16 x_start, int16 y_start, int16 x_end, int16 y_end, const uint16 color)
{
